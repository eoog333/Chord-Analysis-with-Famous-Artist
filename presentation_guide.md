...
### 3.5. K-Means 군집화 (Clustering) 및 클러스터 프로파일링
...
8차원 화성 데이터를 분석하여 규명한 세 개의 음악 스타일 군집은 다음과 같습니다.

1. **군집 1: "다이아토닉 중심의 우울한 단조 그룹" (Diatonic Melancholic)**
   * **특징:** 높은 마이너 비율(`feat_minor_ratio`)과 극도로 낮은 비다이아토닉 비율(`feat_non_diatonic_ratio`).
   * **해석:** 장조의 밝음보다는 마이너 코드의 서정성과 감수성을 적극 차용하되, 기본 조성의 스케일 밖으로 나가는 변칙(차용화음)을 지양하여 대중적이고 듣기 편안한 코드 전개를 이룹니다. 서정적인 발라드나 모던 락 아티스트들이 속합니다.

2. **군집 2: "차용화음을 적극 활용하는 실험적 그룹" (Experimental / Non-Diatonic)**
   * **특징:** 압도적으로 높은 비다이아토닉 비율(`feat_non_diatonic_ratio`)과 고유 코드 비율(`feat_unique_chords`).
   * **해석:** 조성의 틀에 갇히지 않고 세컨더리 도미넌트나 모달 인터체인지 등 음악적 전조와 변칙적 코드 진행을 자주 사용하여 몽환적이거나 극적인 긴장감을 연출합니다. 라디오헤드(Radiohead)와 같은 실험적이고 작곡의 깊이가 있는 얼터너티브/프로그레시브 아티스트들이 포함됩니다.

3. **군집 3: "텐션 코드가 풍부한 세련된 그룹" (Tension-Rich Sophisticated)**
   * **특징:** 아주 높은 텐션 코드 비율(`feat_tension_ratio`)과 발달한 루프 지수(`feat_loop_ratio`).
   * **해석:** 3화음 중심의 단순함을 탈피하여 7도, 9도 등 세련된 컬러 톤(Tension)을 얹은 화음을 적극 사용합니다. 화성 진행의 극적인 변화보다는 짧은 코드 루프를 반복하면서 코드 자체의 사운드 질감을 화려하게 매만집니다. 알앤비, 네오소울, 재즈 팝, 현대 시티팝 성향의 아티스트들이 이에 해당합니다.
...
### 4.3. 코사인 유사도 상위 아티스트 쌍이 90% 이상으로 높게 나오는 원인
아티스트들의 8차원 화성 분석 벡터를 비교한 결과, 특정 아티스트 쌍들이 **90%가 넘는 높은 화성적 스타일 유사도**를 보였습니다. 이는 다음과 같은 수학적 및 음악적 이유에서 기인합니다.

1. **수학적 요인 (정규화된 양수 피처 공간):**
   * 모든 피처가 `MinMaxScaler`를 통해 $[0, 1]$ 범위의 양수로 변환되었습니다. 8차원 벡터 공간에서 모든 데이터가 1사분면(Positive Orthant) 내에 모여 있기 때문에, 두 벡터 사이의 사잇각($\theta$)이 매우 좁아져 코사인 유사도($\cos(\theta)$)가 1(100%)에 가깝게 수렴합니다.
   * 또한 개별 곡이 아닌 아티스트별 곡들의 **평균(Mean) 벡터**를 분석 대상으로 삼았기 때문에, 극단적인 변칙 진행들이 상쇄되어 평균값들 간의 거리가 더 밀접해집니다.

2. **음악적 요인 (대중음악 화성의 보편적 공유):**
   * 대중음악은 장르에 상관없이 듣기 편안한 화성 진행 규칙(예: 30~60%의 마이너 비율, 일정한 순차/도약 진행 비율)을 따릅니다.
   * 아티스트들의 스타일적 차이는 화성 진행의 '근본적인 방향 전환'이라기보다 '미세한 텐션 조율 및 반복 수준의 조절'에 가깝기 때문에, 음악적 특성이 90% 이상 유사하게 분석되는 것입니다.
